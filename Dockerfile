...
COPY . /root/adaptyst/
RUN emerge -g --quiet-build=y llvm-core/clang && cd /root/adaptyst && ./build.sh && { echo | PATH=$PATH:$(dirname /usr/lib/llvm/*/bin/clang) ./install.sh; } && cd .. && rm -rf adaptyst && emerge --deselect llvm-core/clang && emerge --depclean
